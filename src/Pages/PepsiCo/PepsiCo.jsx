...
const PepsiCo = () => {
  ...
  const pepsicoProduct = allProduct.filter(
    (product) => product.brandName.toLowerCase() == "pepsico".toLowerCase()
  );

  return (
    <>
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-w-screen-xl mx-auto mt-6">
        {pepsicoProduct.map((item) => {
      ...
      <AdvertiseBanner title={"pepsico"}></AdvertiseBanner>
    </>
  );
};
